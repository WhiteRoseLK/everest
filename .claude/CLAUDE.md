...
- Le home directory de l'utilisateur `node` (`/home/node`) n'était monté sur aucun volume : un simple `restart: unless-stopped` (redémarrage du même conteneur) préservait l'état d'onboarding/config de Claude Code, mais toute _recréation_ du conteneur (`docker compose down`/`up`, rebuild d'image) le perdait — reposant les questions de connexion/personnalisation initiale même avec `CLAUDE_CODE_OAUTH_TOKEN` déjà fourni. `docker-compose.yml` monte désormais un volume nommé `claude_home` sur `/home/node` (issue #63) : Docker le seede depuis le contenu déjà présent dans l'image (y compris le `.gitconfig` écrit au build) lors du tout premier démarrage, puis le préserve à travers les recréations suivantes.
- **Ownership du repo bind-monté en Docker rootless (issue #84)** : le bind mount `.:/app` fait apparaître le repo (propriété de l'utilisateur hôte, ex. `ubuntu` uid 1000) comme appartenant à **root** _à l'intérieur_ du conteneur en Docker rootless (root conteneur = utilisateur hôte ; `node` = un sous-uid décalé). L'utilisateur `node` (non-root, imposé par `bypassPermissions`) ne peut donc pas écrire dans `/app/.git` : `git checkout`/`commit`/`fetch` échouent tous en `EACCES` et **toute la boucle est bloquée** avant même de choisir une issue (diagnostiqué en live via `everest doctor` + `.harness/errors.jsonl`, voir issue #82). Le `chown` du build (Dockerfile) n'y peut rien : le bind mount remplace l'ownership de `/app` au runtime. Solution : le conteneur **démarre en root** (= l'utilisateur hôte en rootless) via un `ENTRYPOINT` (`docker-entrypoint.sh`) qui réaligne l'ownership de `/app` sur `node` (`find /app -xdev ! -user node -exec chown node:node`, `-xdev` pour ne pas parcourir les volumes montés par-dessus), puis **droppe vers `node` avec `gosu`** avant de lancer `npm start`. Comme le conteneur ne finit plus sur `USER node`, tous les `docker compose exec` (alias `everest` dans `setup.sh`, `runChat` dans `src/cli.ts`) passent explicitement `-u node`, sinon ils tourneraient en root (refusé par `bypassPermissions`). `ENV HOME=/home/node` est fixé car ni `gosu` ni `docker exec -u` ne définissent `$HOME` (sans quoi git/gh/claude chercheraient leur config dans `/root`). Effet de bord assumé (décision explicite de l'utilisatrice, 20 juillet 2026) : après réalignement, le repo côté hôte appartient au sous-uid de `node`, donc l'opérateur n'édite plus le repo directement sur l'hôte (il passe par le conteneur ou `sudo`) — acceptable car le harnais gère le repo lui-même et s'auto-met à jour (issue #43). Alternative écartée : passer Docker en rootful (zéro effet de bord mais reconfiguration du démon + moins d'isolation).
